...
fun AnalyticsScreen() {
    ...
    LaunchedEffect(Unit) {
        vm.syncAndGetInstalledApps(15)
        vm.fetchAndGenerateChartJson(15)
    }
    if (chartJson.isNotBlank()) {
        ...
        Column(modifier = Modifier
            .fillMaxSize()
            .padding(bottom = 8.dp)) {
            // Chart + Legend in one column block
            Column(
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(1f),
        ...
        Box(
            ...
            Text("Loading chart...", color = androidx.compose.ui.graphics.Color.DarkGray)
        }
    }
}
